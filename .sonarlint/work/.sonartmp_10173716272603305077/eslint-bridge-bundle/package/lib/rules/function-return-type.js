    ...
    create(context) {
        ...
        function onFunctionExit(node) {
            ...
            if (signature && hasMultipleReturnTypes(signature, checker)) {
                const stmts = returnStatements.filter(retStmt => !isNullLike((0, utils_1.getTypeFromTreeNode)(retStmt.argument, services)));
                const stmtsTypes = stmts.map(retStmt => (0, utils_1.getTypeFromTreeNode)(retStmt.argument, services));
                if (stmtsTypes.every(utils_1.isAny)) {
                    return;
                }
                context.report({
                    message: (0, utils_1.toEncodedMessage)('Refactor this function to always return the same type.', stmts, stmtsTypes.map(stmtType => `Returns ${prettyPrint(stmtType, checker)}`)),
                    loc: (0, locations_1.getMainFunctionTokenLocation)(node, (0, utils_1.getParent)(context), context),
                });
            }
        }
        return {
