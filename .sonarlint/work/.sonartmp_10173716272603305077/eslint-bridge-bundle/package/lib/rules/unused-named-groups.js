...
function checkStringReplaceGroupReferences(callExpr, intellisense) {
    if ((0, utils_string_replace_1.isStringReplaceCall)(callExpr, intellisense.services)) {
        const [pattern, substr] = callExpr.arguments;
        const regex = intellisense.findRegex(pattern);
        if (regex) {
            const references = (0, utils_string_replace_1.extractReferences)(substr);
            ...
            if (indexedGroups.length > 0) {
                intellisense.context.report({
                    message: (0, utils_1.toEncodedMessage)(`Directly use the group names instead of their numbers.`, indexedGroups.map(group => ({
                        loc: (0, utils_1.getRegexpLocation)(regex.node, group.node, intellisense.context),
...
function checkIndexBasedGroupReference(memberExpr, intellisense) {
    ...
    if (regex) {
        const maybeIndex = (0, utils_1.getValueOfExpression)(intellisense.context, property, 'Literal');
        if (maybeIndex && typeof maybeIndex.value === 'number') {
            ...
            if (group) {
                group.used = true;
                intellisense.context.report({
                    message: (0, utils_1.toEncodedMessage)(`Directly use '${group.name}' instead of its group number.`, [{ loc: (0, utils_1.getRegexpLocation)(regex.node, group.node, intellisense.context) }], [`Group '${group.name}'`]),
                    node: property,
                });
            }
        }
    ...
    if (regex) {
        ...
        for (const groupNode of groupNodes) {
            ...
            if (group) {
                ...
                intellisense.context.report({
                    message: (0, utils_1.toEncodedMessage)(`There is no group named '${groupNode.name}' in the regular expression.`, regex.groups.map(grp => ({
                        loc: (0, utils_1.getRegexpLocation)(regex.node, grp.node, intellisense.context),
...
function extractGroupNodes(memberExpr, intellisense) {
    if ((0, utils_1.isDotNotation)(memberExpr)) {
        ...
        if (parent) {
            switch (property.name) {
                ...
                case 'indices':
                    ...
                    if ((0, utils_1.isDotNotation)(parent) && parent.property.name === 'groups') {
...
function extractNamedOrDestructuredGroupNodes(node) {
    if ((0, utils_1.isDotNotation)(node)) {
        /* matcher.groups.<name> */
        return [node.property];
    }
    else if ((0, utils_1.isObjectDestructuring)(node)) {
    ...
    intellisense.getKnowledge().forEach(regex => {
        if (regex.matched) {
            ...
            if (unusedGroups.length) {
                intellisense.context.report({
                    message: (0, utils_1.toEncodedMessage)('Use the named groups of this regex or remove the names.', unusedGroups.map(grp => ({
                        loc: (0, utils_1.getRegexpLocation)(regex.node, grp.node, intellisense.context),
    ...
    intellisense.getKnowledge().forEach(regex => {
        regex.groups.forEach(group => group.node.references.forEach(reference => {
            if (typeof reference.ref === 'number') {
                intellisense.context.report({
                    message: (0, utils_1.toEncodedMessage)(`Directly use '${group.name}' instead of its group number.`, [{ loc: (0, utils_1.getRegexpLocation)(regex.node, group.node, intellisense.context) }], [`Group '${group.name}'`]),
                    loc: (0, utils_1.getRegexpLocation)(regex.node, reference, intellisense.context),
                });
            }
        }));
    });
...
class RegexIntelliSense {
    ...
    collectKnowledge(node) {
        let regexNode = node;
        if (node.type === 'CallExpression' && (0, utils_1.isStringRegexMethodCall)(node, this.services)) {
            /* implicit regex */
            regexNode = node.arguments[0];
        }
        const regex = (0, utils_1.getParsedRegex)(regexNode, this.context);
        if (regex !== null) {
    ...
    collectPatternMatcher(callExpr) {
        ...
        if ((0, utils_1.isMethodCall)(callExpr) && args.length > 0) {
            const target = callee.object;
            const matcher = (0, utils_1.getLhsVariable)(this.context);
            if (matcher) {
                const method = callee.property;
                if ((0, utils_1.isString)(target, this.services) && ['match', 'matchAll'].includes(method.name)) {
                    /* str.match(pattern) */
                    const [pattern] = args;
                    this.bind(pattern, matcher);
                }
                else if (method.name === 'exec' && (0, utils_1.isString)(args[0], this.services)) {
                    /* pattern.exec(str) */
    ...
    findRegexRec(node, visited) {
        if (!visited.has(node)) {
            ...
            if (variable) {
                const value = (0, utils_1.getUniqueWriteUsage)(this.context, variable.name);
                if (value) {
    ...
    findVariable(node) {
        if (node.type === 'Identifier') {
            return (0, utils_1.getVariableFromName)(this.context, node.name);
        }
        return null;
    }
}
