    ...
    create(context) {
        ...
        return {
            'NewExpression[callee.type!="ThisExpression"]': (node) => {
                const { callee } = node;
                const type = (0, utils_1.getTypeFromTreeNode)(callee, services);
                const signature = (0, utils_1.getSignatureFromCallee)(node, services);
                if (!isInstantiable(type, signature, considerJSDoc) && !isAny(type)) {
                    ...
                    context.report({
                        message: (0, utils_1.toEncodedMessage)(`Replace ${text} with a constructor function.`, [newToken]),
                        loc,
                    });
                }
            },
