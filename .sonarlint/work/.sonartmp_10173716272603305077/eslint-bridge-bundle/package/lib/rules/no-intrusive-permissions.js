        ...
        return {
            'CallExpression[callee.type="MemberExpression"]'(node) {
                ...
                if (isNavigatorMemberExpression(callee, 'mediaDevices', 'getUserMedia') &&
                    call.arguments.length > 0) {
                    const firstArg = (0, utils_1.getValueOfExpression)(context, call.arguments[0], 'ObjectExpression');
                    checkForCameraAndMicrophonePermissions(context, callee, firstArg);
                    return;
                }
                if (context.options.includes('notifications') &&
                    (0, utils_1.isMemberExpression)(callee, 'Notification', 'requestPermission')) {
                    context.report({
                ...
                if (context.options.includes('persistent-storage') &&
                    (0, utils_1.isMemberExpression)(callee.object, 'navigator', 'storage')) {
                    context.report({
            ...
            NewExpression(node) {
                const { callee } = node;
                if (context.options.includes('notifications') && (0, utils_1.isIdentifier)(callee, 'Notification')) {
                    context.report({
                        message: 'Make sure the use of the notifications is necessary.',
                        node: callee,
                    });
...
function checkForCameraAndMicrophonePermissions(context, callee, firstArg) {
    ...
    for (const prop of firstArg.properties) {
        if (prop.type === 'Property') {
            ...
            if ((0, utils_1.isIdentifier)(key, 'audio') && shouldCheckAudio && isOtherThanFalse(context, value)) {
                perms.push('microphone');
            }
            else if ((0, utils_1.isIdentifier)(key, 'video') &&
                shouldCheckVideo &&
...
function isOtherThanFalse(context, value) {
    const exprValue = (0, utils_1.getValueOfExpression)(context, value, 'Literal');
...
function isNavigatorMemberExpression({ object, property }, firstProperty, ...secondProperty) {
    return ((0, utils_1.isMemberExpression)(object, 'navigator', firstProperty) &&
        (0, utils_1.isIdentifier)(property, ...secondProperty));
}
function hasNamePropertyWithPermission(prop, context) {
    if (prop.type === 'Property' && (0, utils_1.isIdentifier)(prop.key, 'name')) {
        const value = (0, utils_1.getValueOfExpression)(context, prop.value, 'Literal');
        return (value &&
