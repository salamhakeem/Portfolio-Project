...
const reachingDefinitions_1 = require("./reachingDefinitions");
const message = (name) => `Review this redundant assignment: "${name}" already holds the assigned value along all execution paths.`;
exports.rule = {
    create(context) {
        const codePathStack = [];
        ...
        return {
            ...
            'Program:exit': () => {
                (0, reachingDefinitions_1.reachingDefinitions)(reachingDefsMap);
            ...
            onCodePathSegmentStart: (segment) => {
                reachingDefsMap.set(segment.id, new reachingDefinitions_1.ReachingDefinitions(segment));
            },
        ...
        function checkSegment(reachingDefs) {
            ...
            reachingDefs.references.forEach(ref => {
                ...
                const rhsValues = (0, reachingDefinitions_1.resolveAssignedValues)(variable, ref.writeExpr, assignedValuesMap, ref.from);
        ...
        function reachingDefsForSegment(segment) {
            ...
            if (reachingDefsMap.has(segment.id)) {
                ...
                defs = new reachingDefinitions_1.ReachingDefinitions(segment);
...
function isSelfAssignement(ref) {
    ...
    if (((_a = ref.writeExpr) === null || _a === void 0 ? void 0 : _a.type) === 'Identifier') {
        const rhs = (0, reachingDefinitions_1.getVariableFromIdentifier)(ref.writeExpr, ref.from);
        return lhs === rhs;
    }
    return false;
}
