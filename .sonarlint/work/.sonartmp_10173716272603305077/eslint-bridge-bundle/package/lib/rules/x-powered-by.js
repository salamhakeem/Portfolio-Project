        ...
        return {
            ...
            CallExpression: (node) => {
                if (!isSafe && appInstantiation) {
                    const callExpr = node;
                    isSafe =
                        utils_express_1.Express.isUsingMiddleware(context, callExpr, appInstantiation, isProtecting(context)) ||
            ...
            VariableDeclarator: (node) => {
                if (!isSafe && !appInstantiation) {
                    const varDecl = node;
                    const app = utils_express_1.Express.attemptFindAppInstantiation(varDecl, context);
                    if (app) {
...
function isHidePoweredByFromHelmet(context, n) {
    ...
    if (n.type === 'CallExpression') {
        ...
        return (callee.type === 'MemberExpression' &&
            ((_a = (0, utils_1.getModuleNameOfNode)(context, callee.object)) === null || _a === void 0 ? void 0 : _a.value) === HELMET &&
...
function isProtecting(context) {
    return (n) => utils_express_1.Express.isMiddlewareInstance(context, PROTECTING_MIDDLEWARES, n) ||
        isHidePoweredByFromHelmet(context, n);
}
function isDisabledXPoweredBy(callExpression, app) {
    if ((0, utils_1.isMethodInvocation)(callExpression, app.name, 'disable', 1)) {
...
function isSetFalseXPoweredBy(callExpression, app) {
    if ((0, utils_1.isMethodInvocation)(callExpression, app.name, 'set', APP_SET_NUM_ARGS)) {
