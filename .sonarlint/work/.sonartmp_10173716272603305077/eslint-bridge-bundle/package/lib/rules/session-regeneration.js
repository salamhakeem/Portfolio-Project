    ...
    create(context) {
        ...
        function isSessionRegenerate(node) {
            return (node.type === 'CallExpression' &&
                ...
                (0, utils_1.isIdentifier)(node.callee.property, 'regenerate'));
        }
        function visitCallback(node) {
            ...
            (0, utils_1.childrenOf)(node, context.getSourceCode().visitorKeys).forEach(visitCallback);
        }
        function hasSessionFalseOption(callExpression) {
            ...
            if ((opt === null || opt === void 0 ? void 0 : opt.type) === 'ObjectExpression') {
                const sessionProp = (0, utils_1.getPropertyWithValue)(context, opt, 'session', false);
                return !!sessionProp;
            }
            return false;
        }
        return {
            CallExpression: (node) => {
                ...
                if ((0, utils_1.isCallToFQN)(context, callExpression, 'passport', 'authenticate')) {
                    if (hasSessionFalseOption(callExpression)) {
                        return;
                    }
                    const parent = (0, utils_1.last)(context.getAncestors());
                    if (parent.type === 'CallExpression') {
                        const callback = (0, utils_1.getValueOfExpression)(context, parent.arguments[2], 'FunctionExpression');
