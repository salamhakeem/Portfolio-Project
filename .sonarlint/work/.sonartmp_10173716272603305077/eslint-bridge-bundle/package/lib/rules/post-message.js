...
function isWindowObject(node, context) {
    const type = (0, utils_1.getTypeAsString)(node, context.parserServices);
...
function checkPostMessageCall(callExpr, context) {
    ...
    if (![2, 3].includes(callExpr.arguments.length) ||
        ((_a = (0, utils_1.getValueOfExpression)(context, callExpr.arguments[1], 'Literal')) === null || _a === void 0 ? void 0 : _a.value) !== '*') {
        return;
    }
...
function checkAddEventListenerCall(callExpr, context) {
    ...
    const listener = (0, utils_1.resolveFunction)(context, args[1]);
...
function isMessageTypeEvent(eventNode, context) {
    const eventValue = (0, utils_1.getValueOfExpression)(context, eventNode, 'Literal');
...
class WindowNameVisitor {
    ...
    visit(root, context) {
        const visitNode = (node) => {
            ...
            (0, utils_1.childrenOf)(node, context.getSourceCode().visitorKeys).forEach(visitNode);
        };
        visitNode(root);
    }
}
