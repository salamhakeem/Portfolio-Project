    ...
    create(context) {
        ...
        return {
            'BinaryExpression[operator="+"]'(node) {
                ...
                if ((0, utils_1.isStringLiteral)(left) ||
                    (0, utils_1.isStringLiteral)(right) ||
                    isConcatenation(left) ||
                    isConcatenation(right)) {
                    return;
                }
                const leftType = (0, utils_1.getTypeFromTreeNode)(left, services);
                const rightType = (0, utils_1.getTypeFromTreeNode)(right, services);
                if (isStringPlusNonString(leftType, rightType) ||
                    isStringPlusNonString(rightType, leftType)) {
                    context.report({
                        message: (0, utils_1.toEncodedMessage)(message, [left, right], [
