...
exports.rule = {
    create(context) {
        ...
        return {
            CallExpression: (call) => {
                if ((0, utils_string_replace_1.isStringReplaceCall)(call, services)) {
                    const [pattern, substr] = call.arguments;
                    const regex = (0, utils_1.getParsedRegex)(pattern, context);
                    if (regex !== null) {
                        ...
                        if (invalidReferences.length > 0) {
                            const message = `Referencing non-existing group${invalidReferences.length > 1 ? 's' : ''}: ${invalidReferences.map(ref => ref.raw).join(', ')}`;
                            context.report({
                                node: substr,
                                message,
                            });
