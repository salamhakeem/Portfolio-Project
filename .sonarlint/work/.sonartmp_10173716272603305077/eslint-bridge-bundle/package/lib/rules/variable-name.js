...
function checkVariable(decl, context) {
    ...
    decl.declarations.forEach(declaration => (0, utils_1.resolveIdentifiers)(declaration.id).forEach(id => raiseOnInvalidIdentifier(id, 'local variable', context)));
}
function checkFunction(func, context) {
    if (func.declare) {
        return;
    }
    func.params.forEach(param => (0, utils_1.resolveIdentifiers)(param).forEach(id => raiseOnInvalidIdentifier(id, 'parameter', context)));
}
...
function checkCatch(catchh, context) {
    if (catchh.param) {
        (0, utils_1.resolveIdentifiers)(catchh.param).forEach(id => raiseOnInvalidIdentifier(id, 'parameter', context));
    }
}
