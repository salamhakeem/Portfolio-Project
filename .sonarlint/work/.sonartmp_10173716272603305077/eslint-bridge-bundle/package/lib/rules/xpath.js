    ...
    create(context) {
        return {
            MemberExpression: (node) => {
                if ((0, utils_1.isMemberExpression)(node, 'document', 'evaluate')) {
...
function checkCallExpression({ callee, arguments: args }, context) {
    if (args.length > 0 && (0, utils_1.isLiteral)(args[0])) {
        return;
    }
    // IE
    if ((0, utils_1.isMemberWithProperty)(callee, ...ieEvalMethods) && args.length === 1) {
        context.report({ message, node: callee });
        return;
    }
    // Document.evaluate
    if ((0, utils_1.isMemberWithProperty)(callee, 'evaluate') &&
        !(0, utils_1.isMemberExpression)(callee, 'document', 'evaluate') &&
        args.length >= 4) {
    ...
    // "xpath" module
    const { module, method } = (0, utils_1.getModuleAndCalledMethod)(callee, context);
    if (method &&
        module &&
