...
exports.rule = {
    ...
    create(context) {
        ...
        function haveDissimilarTypes(lhs, rhs) {
            ...
            const lhsType = getBaseTypeOfLiteralType((0, utils_1.getTypeFromTreeNode)(lhs, services));
            const rhsType = getBaseTypeOfLiteralType((0, utils_1.getTypeFromTreeNode)(rhs, services));
        ...
        return {
            BinaryExpression: (node) => {
                ...
                if (['===', '!=='].includes(operator) && haveDissimilarTypes(left, right)) {
                    ...
                    context.report({
                        message: (0, utils_1.toEncodedMessage)(`Remove this "${actual}" check; it will always be ${outcome}. Did you mean to use "${expected}"?`, [left, right]),
                        loc: context
                            .getSourceCode()
