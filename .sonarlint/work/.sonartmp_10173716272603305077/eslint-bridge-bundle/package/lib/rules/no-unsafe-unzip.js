    ...
    create(context) {
        function canBeProperty(prop, name) {
            return (prop.type === 'SpreadElement' ||
                (0, utils_1.isIdentifier)(prop.key, name) ||
                ((0, utils_1.isLiteral)(prop.key) && prop.key.value === name));
        }
        function isCallToAdmZipExtractAll({ callee }) {
            return callee.type === 'MemberExpression' && (0, utils_1.isIdentifier)(callee.property, 'extractAllTo');
        }
        function isAdmZipLibraryInScope() {
            return isAdmZipLibraryImported() || isAdmZipLibraryRequired();
        }
        function isAdmZipLibraryImported() {
            return (0, utils_1.getImportDeclarations)(context).some(i => i.source.value === ADM_ZIP);
        }
        function isAdmZipLibraryRequired() {
            return (0, utils_1.getRequireCalls)(context).some(r => r.arguments[0].type === 'Literal' && r.arguments[0].value === ADM_ZIP);
        }
        function isSensiteTarCall(call) {
            if ((0, utils_1.isCallToFQN)(context, call, 'tar', 'x')) {
                const firstArg = call.arguments.length > 0 ? call.arguments[0] : null;
                if (!firstArg) {
                    return false;
                }
                const firstArgValue = (0, utils_1.getValueOfExpression)(context, firstArg, 'ObjectExpression');
        ...
        function isSensiteExtractZipCall(call) {
            ...
            if (call.callee.type === 'Identifier' &&
                ((_a = (0, utils_1.getModuleNameOfIdentifier)(context, call.callee)) === null || _a === void 0 ? void 0 : _a.value) === 'extract-zip') {
                const secondArg = call.arguments.length > 1 ? call.arguments[1] : null;
                if (!secondArg) {
                    return false;
                }
                const secondArgValue = (0, utils_1.getValueOfExpression)(context, secondArg, 'ObjectExpression');
        ...
        return {
            CallExpression(node) {
                ...
                if (isSensiteTarCall(call) ||
                    ...
                    (0, utils_1.isCallToFQN)(context, call, 'jszip', 'loadAsync') ||
                    (0, utils_1.isCallToFQN)(context, call, 'yauzl', 'open') ||
