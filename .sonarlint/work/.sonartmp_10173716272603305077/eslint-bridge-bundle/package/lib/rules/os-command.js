...
function checkCallExpression({ callee, arguments: args }, context) {
    if (callee.type === 'MemberExpression') {
        if (callee.object.type === 'Identifier') {
            const moduleName = (0, utils_1.getModuleNameOfIdentifier)(context, callee.object);
...
function isQuestionable(expression, [command, ...otherArguments]) {
    ...
    if ((0, utils_1.isIdentifier)(expression, ...SPAWN_EXEC_FILE_FUNCTIONS)) {
        return containsShellOption(otherArguments);
    }
    return (0, utils_1.isIdentifier)(expression, ...EXEC_FUNCTIONS);
}
function containsShellOption(otherArguments) {
    return otherArguments.some(arg => arg.type === 'ObjectExpression' &&
        arg.properties.filter(v => v.type === 'Property').some(({ key, value }) => (0, utils_1.isIdentifier)(key, 'shell') && value.type === 'Literal' && value.value === true));
}
